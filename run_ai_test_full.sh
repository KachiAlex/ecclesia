#!/bin/bash
# Temporarily set a known password for testing, run tests, then restore
BASE="http://127.0.0.1:3002"
COOKIE_JAR="/tmp/ai_test_cookies2.txt"
rm -f "$COOKIE_JAR"

# Set a temporary known password using bcrypt hash for "TestPass123!"
# $2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy
TEMP_HASH='$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy'
...
echo ""
echo "[SETUP] Setting temporary test password..."
...
LOGIN_RESPONSE=$(curl -s -b "$COOKIE_JAR" -c "$COOKIE_JAR" \
  ...
  -d "email=onyedika.akoma@gmail.com&password=TestPass123!&csrfToken=$CSRF_TOKEN&callbackUrl=http://127.0.0.1:3002/dashboard&json=true" \
...
echo "  Session: $SESSION_RESPONSE" | head -c 300
echo ""
...
if echo "$SESSION_RESPONSE" | grep -q '"user"'; then
  ...
  echo "  Restoring password and exiting..."
  docker exec -i ecclesia-db psql -U ecclesia -d ecclesia -c "UPDATE public.\"User\" SET password = '$ORIG_HASH' WHERE email = 'onyedika.akoma@gmail.com'" > /dev/null
  echo "  Password restored"
  rm -f "$COOKIE_JAR"
  exit 1
...
echo "  Response: $NOTIF_BODY" | head -c 500
echo ""
