    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold">Reading Plans &amp; Library</h1>
          <p className="text-gray-600 mt-1">
            Explore guided plans, daily scriptures, devotionals, and the shared digital library from your church.
          </p>
        </div>
        {activeTab === 'plans' && (
          <button
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-sm uppercase tracking-widest text-white/70">
                  {dailyReading
                    ? new Date(dailyReading.date).toLocaleDateString(undefined, {
                        weekday: 'long',
                        month: 'long',
                        day: 'numeric',
                      })
                ...
                {plans.map((plan) => (
                  <div key={plan.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
                    ...
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="text-sm text-gray-500">{plan._count.progress} participants</div>
                    </div>
                          ...
                          {!plan.userProgress.completed && (
                            <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                              <div
                                ...
                                style={{ width: `${(plan.userProgress.currentDay / plan.duration) * 100}%` }}
                              ></div>
                            </div>
                          )}
                        </div>
